...
WHERE username= ?";
$stmt = $conn->prepare($query);$stmt->execute([$_SESSION["username"]]);
    ...
    <div class="col s12 m6 l6">
      <div class="card">
        ...
        <div class="card-action">
          <a href="product-view.php?id='. $row["img"] .'" class="btn-flat waves-effect">see more</a>
        </div>
      </div>
    </div>';
    }
...
$query = "SELECT * FROM shoppingCart
JOIN products 
ON products.product_id = shoppingCart.product_id 
WHERE username= ?";
$stmt = $conn->prepare($query); $stmt->execute([$_SESSION["username"]]);
if($stmt->rowCount() > 0 ) {
  while($row = $stmt->fetchAll(PDO::FETCH_ASSOC)) {
